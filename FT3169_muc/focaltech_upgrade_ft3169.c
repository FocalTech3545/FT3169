...
*       Secondly, modify variable fw_file_ft3169 to include firmware file you put into firmware directory.
...
#define FTS_CMD_FW_START_ADDRESS                    0x00
/*register address*/
...
#define FTS_VER_IN_FILE_ADDR                        0x010E
...
/* The array variable fw_file_ft3169 stores the firmware file that you
 * want to upgrade.
...
static int fts_fwupg_ecc_tp(uint32_t saddr, uint16_t len)
{
    ...
    ret = fts_write(wbuf[0], &wbuf[1] , FTS_CMD_ECC_CAL_LEN);
    if (ret < 0) {
...
/*****************************************************************************
* Name: fts_ft3169_upgrade
* Brief:
*   Main flow of FT3169 firmware upgrade.
...
*****************************************************************************/
static int fts_ft3169_upgrade(uint8_t *fw_file, uint32_t fw_file_size)
{
    int ret = 0;
    ...
    if (!fw_file || (fw_file_size < FTS_MIN_LEN) || (fw_file_size > FTS_MAX_LEN_APP)) {
...
static int fts_fwupg_upgrade(uint8_t *fw_file, uint32_t fw_file_size)
{
    ...
    if (upgrade_flag) {
        do {
            upgrade_count++;
            ret = fts_ft3169_upgrade(fw_file, fw_file_size);
            if (ret >= 0) {
...
{
    int ret = 0;
    uint8_t *fw_file = fw_file_ft3169;
    uint32_t fw_file_size = sizeof(fw_file_ft3169);

    FTS_INFO("********************auto upgrade********************");
    /* Get firmware file, and check firmware size
     * we use all.i as an example(variable fw_file_ft3169 stores the firmware).
